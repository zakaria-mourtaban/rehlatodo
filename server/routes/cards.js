...
router.post("/", async (req, res) => {
	...
	try {
		...
		// Validation - ensure column_id is provided
		if (!column_id) {
...
router.put("/:id", async (req, res) => {
	...
	try {
		...
		// Store the card title - use current title if not provided in update
		const cardTitle = title || currentCard.title;

		// Get new column name if changed
		let toColumn = fromColumn;
		if (column_id !== undefined && column_id !== currentCard.columnId) {
		...
		if (
			(column_id === undefined || column_id === currentCard.columnId) &&
			position !== undefined && 
		...
		// Handle moving to a different column
		else if (column_id !== undefined && column_id !== currentCard.columnId) {
			...
			await Card.increment("position", {
				...
				where: {
					columnId: currentCard.columnId,
					position: { [Op.gt]: currentCard.position },
				},
				transaction,
			});
		}
		...
		if (column_id !== undefined) updateData.columnId = column_id;
		if (position !== undefined) updateData.position = position;
		if (tag_id !== undefined) updateData.tagId = tag_id;

		await currentCard.update(updateData, { transaction });
		...
		if (column_id !== undefined && column_id !== currentCard.columnId) {
			...
			actionType =
				position < currentCard.position ? "moved_up" : "moved_down";
		} else {
			actionType = "updated";
		}

		// Create a log entry with explicit cardTitle
		await Log.create(
			{
				cardId: id,
				cardTitle: cardTitle, // Use the stored title
				actionType,
				fromColumn,
				toColumn,
				fromPosition,
				toPosition: position !== undefined ? position : currentCard.position,
				userId,
			},
			{ transaction }
		);
