...
router.put("/:id", async (req, res) => {
	...
	try {
		...
		if (column.userId !== null && column.userId !== userId) {
			...
			return res
				.status(403)
				.json({ error: "Not authorized to update this column" });
		}
			...
			if (position < column.position) {
				...
				await Column.increment("position", {
					...
					where: {
						position: {
							[Op.gte]: position,
							[sequelize.Op.lt]: column.position,
						},
					},
					transaction,
				});
...
router.delete("/:id", async (req, res) => {
	...
	try {
		...
		if (column.userId !== null && column.userId !== userId) {
			...
			return res
				.status(403)
				.json({ error: "Not authorized to delete this column" });
		}
		...
		await Column.increment("position", {
			by: -1,
			where: {
				position: { [Op.gt]: column.position },
			},
			transaction,
		});
